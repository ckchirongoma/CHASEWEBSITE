...
// Plan Mode Functions (Global scope for onclick handlers)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (planForm) {
        planForm.addEventListener('submit', function(e) {
            ...
            const checkedTools = Array.from(document.querySelectorAll('.tool-checkboxes input:checked')).map(cb => cb.value);
            
            if (taskType && timeSpent) {
                showPlanResult(taskType, timeSpent, checkedTools);
            }
        });
    }
});

function showPlanResult(taskType, timeSpent, tools) {
    ...
    let recommendation = `<div class="plan-recommendation">`;
    recommendation += `<h4>Recommended Solution: ${getTaskSolution(taskType)}</h4>`;
    recommendation += `<p><strong>Time Savings:</strong> ${getTimeSavings(timeSpent)}</p>`;
    recommendation += `<p><strong>Integration Plan:</strong> ${getIntegrationPlan(tools)}</p>`;
    recommendation += `<p><strong>Implementation Time:</strong> ${getImplementationTime(taskType)}</p>`;
    recommendation += `<p><strong>Expected ROI:</strong> ${getROI(timeSpent)}</p>`;
    recommendation += `</div>`;
    
    content.innerHTML = recommendation;
}

function getTaskSolution(taskType) {
    const solutions = {
        'email': 'AI Email Processing Agent',
        'data': 'Automated Data Pipeline',
        'scheduling': 'Smart Scheduling Assistant',
        'research': 'Research & Analysis Agent',
        'customer': 'Customer Support Automation',
        'other': 'Custom Workflow Automation'
    };
    return solutions[taskType] || 'Custom AI Solution';
}

function getTimeSavings(timeSpent) {
    const savings = {
        '5-10': '4-8 hours/week (80% reduction)',
        '10-20': '8-16 hours/week (80% reduction)',
        '20-40': '16-32 hours/week (80% reduction)',
        '40+': '32+ hours/week (80% reduction)'
    };
    return savings[timeSpent] || 'Significant time savings expected';
}

function getIntegrationPlan(tools) {
    if (tools.length === 0) return 'We\'ll assess your current tools during discovery';
    return `Direct integration with ${tools.join(', ')} + API connections`;
}

function getImplementationTime(taskType) {
    return '30-60 days from pilot to production';
}

function getROI(timeSpent) {
    const roi = {
        '5-10': '300-500% ROI within 6 months',
        '10-20': '400-600% ROI within 6 months',
        '20-40': '500-700% ROI within 6 months',
        '40+': '600-800% ROI within 6 months'
    };
    return roi[timeSpent] || 'Significant ROI expected within 6 months';
}
